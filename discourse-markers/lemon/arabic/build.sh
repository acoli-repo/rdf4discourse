#!/bin/bash
# build ar.ttl with default settings
...
SRC=https://raw.githubusercontent.com/discourse-lab/Connective-Lex.info/master/Original%20lexicons/Arabic%20Discours%20cues.xml;
DIMLEX=arabic.xml;
...
if [ -e $ABox ]; then
	...
	if [ ! -e $DIMLEX ]; then
		if whereis $SAXON | grep ':.*/' >/dev/null; then # found
			wget $SRC -O - | saxon -s:- -xsl:arabic2dimlex.xsl  > $DIMLEX;
		else
			echo . 1>&2;
